...
const instrumentRequestSchema = z.object({
	entry: z
		.string({ error: "`entry` must be a non-empty string." })
		.min(1, "`entry` must be a non-empty string."),
	compileCommand: z.union([z.string(), z.array(z.string())]).optional(),
});
...
const instrumentRoutes: FastifyPluginAsync<InstrumentRoutesOptions> = async (
	app,
	options,
) => {
	app.post("/run", async (req, reply) => {
		const body = parseBody(instrumentRequestSchema, req.body ?? {});

		const projectRoot = path.resolve(
			options.projectRoot ?? PROJECT_ROOT_DEFAULT,
		);
		const entry = normaliseRelative(body.entry);
		if (!entry.endsWith(".ts")) {
			throw new HttpError(
				400,
				`Entry must be a .ts file (got "${body.entry}").`,
			);
		}
		...
		if (!isInsideRoot(projectRoot, sourceAbs)) {
			throw new HttpError(
				400,
				`Entry "${body.entry}" escapes the project root.`,
			);
		}
		try {
			...
			throw new HttpError(
				404,
				`Entry file "${entry}" not found in project root ${projectRoot}.`,
			);
		}
		...
		try {
			result = await orchestrate({
				...
				...(body.compileCommand !== undefined
					? {
							compileCommand: body.compileCommand as
								| string
			...
			async start(controller) {
				try {
					...
					if (exitCode !== 0) {
						...
						controller.enqueue(
							encoder.encode(
								`${JSON.stringify({ event: "result", data: { status: "Failed" } })}\n`,
							),
						);
					} else {
						controller.enqueue(
							encoder.encode(
								`${JSON.stringify({ event: "result", data: { status: "Succeeded" } })}\n`,
							),
						);
					}
					controller.close();
