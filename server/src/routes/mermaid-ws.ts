...
export function attachMermaidWebSocket(
  ...
  const onUpgrade = (req: http.IncomingMessage, socket: NodeJS.Socket, head: Buffer): void => {
    ...
    wss.handleUpgrade(req, socket as unknown as import('stream').Duplex, head, (ws) => {
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
      ...
      // subscribe
      void handleSubscribe(ws, msg.entry, options.projectRoot, subscriptions, watchFactory).then(
        (sub) => {
          current = sub
        },
      )
...
async function armWatcher(
  ...
  try {
    ...
    sendSnapshotTo(sub, initial.files.map((f) => f.path), renderProjectFiles(initial.files))
...
async function rebuildAndBroadcast(
  ...
  try {
    ...
    sendSnapshotTo(sub, project.files.map((f) => f.path), renderProjectFiles(project.files))
  } catch (err) {
    sendErrorTo(sub, mapCfgError(err), sub.entry)
  }
}
...
function cleanupSubscription(sub: Subscription, subscriptions: Map<string, Subscription>): void {
  if (sub.rebuildTimer !== null) clearTimeout(sub.rebuildTimer)
  if (sub.handle !== null) sub.handle.close()
...
function sendSnapshotTo(
  ...
  const msg: ServerMessage = { type: 'snapshot', entry: sub.entry, mermaid, files }
...
function isClientMessage(value: unknown): value is ClientMessage {
  ...
  if (v.type === 'subscribe') return typeof v.entry === 'string' && v.entry.length > 0
  return false
}
