...
describe("renderMermaid", () => {
	...
	it("disambiguates node ids across files", () => {
		const a: MermaidInput = {
			path: "a.ts",
			cfg: analyseTypeScript(`function a() {}`),
		};
		const b: MermaidInput = {
			path: "b.ts",
			cfg: analyseTypeScript(`function b() {}`),
		};
	...
	it("keeps node ids stable between declaration and edge references", () => {
      ...
      `),
		...
		const declared = new Set(
			(out.match(/^\s+([A-Za-z_][A-Za-z0-9_]*)\["/gm) ?? []).map(
				(m) => m.trim().split("[")[0]!,
			),
		);
	...
	it("renders a ProjectFile list through renderProjectFiles", () => {
		...
		const files: ProjectFile[] = [
			{ path: "ping.ts", source: "", cfg, imports: [] },
		];
