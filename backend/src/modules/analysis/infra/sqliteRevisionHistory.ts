...
export class SqliteRevisionHistory implements RevisionHistory {
  ...
  async list(scope: ProcedureScope): Promise<readonly RevisionSummary[]> {
    const rows = this.db
      ...
      .all(scope.file, scope.procedureId) as Array<{
      revision: string;
      analyzed_at: string;
    ...
    return rows.map((row) =>
      RevisionSummarySchema.parse({
        ...
        runnable:
          row.cfg_json !== null &&
          this.array(row.diagnostics_json).length === 0,
        diagnosticCount: this.array(row.diagnostics_json).length,
      }),
    );
  }
    ...
    return {
      ...
      release: () => {
        if (!released) {
          ...
          if (count <= 0) this.leases.delete(leaseKey);
          else this.leases.set(leaseKey, count);
        }
      },
    };
  }
  ...
  private prune(file: string, procedureId: string): void {
    ...
    const rows = this.db
      ...
      .all(file, procedureId, cutoff) as Array<{ revision: string }>;
    const newest = new Set(
      (
        this.db
          ...
          .all(file, procedureId) as Array<{ revision: string }>
  ...
  private key(value: RevisionKey): string {
