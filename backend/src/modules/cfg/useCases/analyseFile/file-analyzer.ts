...
export function analyseFileProcedure(
	source: string,
	filePath = "inline.ts",
	options: { showImports?: boolean; functionName?: string } = {},
): ControlFlowGraph {
...
function hasExecutableClassInitialization(
...
function buildTry(
	...
	if (statement.finallyBlock === undefined)
		return { entry, normal: normalSources, abrupt };
			...
			if (ts.isPropertyDeclaration(member))
				parts.push(
					executableNode(
						builder,
						member,
						member.getText(file).trim().replace(/;$/, ""),
						context,
					),
				);
		}
...
function statementLabel(file: ts.SourceFile, statement: ts.Statement): string {
	return statement.getText(file).trim().replace(/;$/, "");
}
function emptyFlow(): Flow {
	return { entry: undefined, normal: [], abrupt: [] };
}
...
function shortCircuitOutcome(kind: ts.SyntaxKind): {
	continue: string;
	stop: string;
} {
	if (
...
function optionalChainParts(
	file: ts.SourceFile,
	expression: ts.Expression,
): { subject: string; access: string } {
	if (ts.isCallExpression(expression)) {
		const subject = expression.expression.getText(file);
		return {
			subject: subject.replace(/\?\.$/, ""),
			access: `${subject.replace(/\?\.$/, "")}()`,
		};
	}
