...
const snapshot: AnalysisSnapshot = {
  ...
  cfg: {
    functions: [],
    procedures: [procedure],
  } as unknown as AnalysisSnapshot["cfg"],
  diagnostics: [],
...
describe("ExecutionManager", () => {
  ...
  it("acquires the exact lease, emits ordered updates, and releases once on success", async () => {
    const release = vi.fn();
    const manager = createExecutionManager(historyFor(release), {
      now: () => new Date("2025-01-01T00:00:00Z"),
      execute,
    });
    const events: unknown[] = [];
    manager.subscribe((event) => events.push(event));
    const id = await manager.start(key);
    runners[0]!.node("n1");
    runners[0]!.node("n2");
    runners[0]!.resolve({ status: "Succeeded", events: ["n1", "n2"] });
    await flush();
    expect(events).toMatchObject([
      { displayNumber: 1, status: "Running" },
      { displayNumber: 1, currentNodeId: "n1" },
      { currentNodeId: "n2" },
      { executionId: id, status: "Succeeded" },
    ]);
  ...
  it("assigns stable display numbers and keeps concurrent runs independent and newest-first", async () => {
    const manager = createExecutionManager(historyFor(), {
      now: () => new Date("2025-01-01T00:00:00Z"),
      execute,
    });
    const first = await manager.start(key);
    const second = await manager.start(key);
    expect(manager.listActive().map((run) => run.displayNumber)).toEqual([
      2, 1,
    ]);
    runners[0]!.resolve({ status: "Succeeded", events: [] });
    await flush();
    expect(manager.listActive().map((run) => run.executionId)).toEqual([
      second,
    ]);
    runners[1]!.resolve({ status: "Succeeded", events: [] });
    await flush();
  ...
  it("cleans up failed and timed-out terminal outcomes", async () => {
    const release = vi.fn();
    const manager = createExecutionManager(historyFor(release), { execute });
    const id = await manager.start(key);
    runners[0]!.node("loop");
    runners[0]!.resolve({
      status: "Failed",
  ...
  it("cancels a run, releases its lease, and returns not-found for unknown IDs", async () => {
    ...
    const manager = createExecutionManager(historyFor(release), { execute });
    const id = await manager.start(key);
    expect(manager.cancel(id)).toBe("cancelled");
    await flush();
    expect(manager.cancel(id)).toBe("not-found");
    expect(manager.cancel("unknown")).toBe("not-found");
    expect(release).toHaveBeenCalledOnce();
    manager.close();
  });
