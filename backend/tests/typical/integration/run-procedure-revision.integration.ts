...
describe("run Procedure revision", () => {
  ...
  it("runs the exact pinned revision after its source file is deleted", async () => {
    ...
    const body = discovered.json() as {
      revision: string;
      procedures: Array<{ id: string }>;
    };
    const analyzed = await app.inject({
      method: "GET",
      url: `/api/analysis?file=main.ts&procedureId=${encodeURIComponent(body.procedures.at(-1)!.id)}`,
    });
    const request = {
      file: "main.ts",
      procedureId: body.procedures.at(-1)!.id,
      revision: (analyzed.json() as { revision: string }).revision,
    };
    await fs.rm(file);
  ...
  it("pins concurrent executions independently to one exact scope", async () => {
    ...
    const responses = await Promise.all(
      [1, 2].map(() =>
        app!.inject({ method: "POST", url: "/api/execute", payload: request }),
      ),
    );
    ...
    const executions = (
      active.json() as {
        executions: Array<{ scope: typeof request; displayNumber: number }>;
      }
    ).executions;
  ...
  it("does not substitute a newer revision when the requested revision is unavailable", async () => {
    ...
    const response = await app.inject({
      method: "POST",
      url: "/api/execute",
      payload: { file: "main.ts", procedureId: "prepare", revision: "missing" },
    });
