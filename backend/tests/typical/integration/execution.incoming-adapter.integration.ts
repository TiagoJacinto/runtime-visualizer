...
describe("execution incoming adapter", () => {
  ...
  async function setup(source: string) {
    ...
    if (analyzed.statusCode !== 200)
      throw new Error(`analysis ${analyzed.statusCode}: ${analyzed.body}`);
  ...
  it("lists active executions and removes them after terminal completion", async () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      if ((listed.json() as { executions: unknown[] }).executions.length === 0)
        break;
  ...
  it("rejects an unavailable exact revision without name-based fallback", async () => {
    ...
    const response = await app!.inject({
      method: "POST",
      url: "/api/execute",
      payload: { ...request, revision: "missing" },
    });
