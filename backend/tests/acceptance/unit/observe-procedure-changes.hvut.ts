...
const feature = await loadFeature(
	new URL("../../../features/observe-procedure-changes.feature", import.meta.url).pathname,
);
	...
	while (!text.includes("data: ")) {
		...
		const result = await Promise.race([
			...
			new Promise<never>((_, reject) =>
				setTimeout(() => reject(new Error("Timed out waiting for file change")), remaining),
			),
		]);
...
describeFeature(feature, ({ Rule }) => {
	...
	Rule("Source changes are observable as additions, modifications, and deletions", ({ RuleScenario, RuleScenarioOutline }) => {
	RuleScenarioOutline(
		"Observe a source file change",
		async ({ Given, When, Then }, example: Record<string, string | undefined>) => {
			...
			When("I observeSourceChanges()", async () => {
				...
				if (example.change === "Added") await fs.writeFile(path.join(folder, "new.ts"), "export const value = 2;\n");
				else if (example.change === "Modified") await fs.writeFile(path.join(folder, "main.ts"), "export const value = 2;\n");
			...
			Then(
				...
				async () => {
					...
					if (expectedChange === "Deleted") expect(change.revision).toBeUndefined();
	...
	RuleScenario("Publish the latest revision with a modification", ({ Given, When, Then }) => {
		let change: FileChange;
		Given('Source file{path: "main.ts", revision: "R1", source: "function prepare() {}"}', async () => {
			folder = await fs.mkdtemp(path.join(os.tmpdir(), "runtime-visualizer-"));
		...
		Then(
			...
			() => {
				// result verification
				expect(change).toMatchObject({ file: "main.ts", change: "modified" });
