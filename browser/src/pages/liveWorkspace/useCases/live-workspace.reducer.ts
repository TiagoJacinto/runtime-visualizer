// Reducer helpers are mutually recursive through transition projections.
...
const transition = (
  ...
  effects: readonly WorkspaceEffect[] = []
): Transition => ({ effects, state: project(state, patch) });
const statusForPane = (
  pane: LiveWorkspaceState["pane"],
  files: readonly string[]
): LiveWorkspaceState["status"] => {
  if (files.length === 0 && pane.status === "empty") {
    return "empty";
  }
...
const compareExecutions = (a: ExecutionRecord, b: ExecutionRecord): number => {
...
const project = (
  ...
  const active = Object.values(next.activeExecutionsById).toSorted(
    compareExecutions
  );
  const completed = next.completedExecutions.toSorted(compareExecutions);
  let analysis: AnalysisResponse | null = null;
  if (next.pane.status === "ready") {
    analysis = next.pane.value;
...
const requestId = (key: RevisionKey): string =>
  `${key.file}\0${key.procedureId}\0${key.revision}`;
const revisionIsSelected = (
...
const sourceChangeTransition = (
  ...
  if (change.change === "added") {
    const files = state.files.includes(change.file)
      ? state.files
      : [...state.files, change.file].toSorted((a, b) => a.localeCompare(b));
    return transition(state, {
      connectionState: { cursor: id, status: "connected" },
      files,
    });
  }
  if (change.change === "deleted") {
    ...
    if (!activeForFile && nextFile !== undefined) {
      effects.push({ file: nextFile, type: "bootstrap-file" });
    }
    return transition(
      state,
      {
...
const revisionsLoaded = (
  ...
  if (
    ...
    const [first] = event.revisions;
    if (first === undefined) {
      return transition(state, { revisionsByScope });
    }
    const key = { ...event.scope, revision: first.revision };
    return transition(state, { revisionsByScope, selectedScope: key }, [
      { key, requestId: requestId(key), type: "load-analysis" },
    ]);
  }
  const [firstRevision] = event.revisions;
  if (selected !== null || firstRevision === undefined) {
    return transition(state, { revisionsByScope });
  }
  const first = firstRevision;
  const key = { ...event.scope, revision: first.revision };
  return transition(state, { revisionsByScope }, [
    { key, requestId: requestId(key), type: "load-analysis" },
  ]);
...
const executionUpdate = (
  ...
  const { [record.executionId]: _active, ...activeExecutionsById } =
    state.activeExecutionsById;
  const { [record.executionId]: _pending, ...pendingById } =
...
const workspaceEvent = (
  ...
  if (workspace.type === "revision-ready") {
    ...
    return transition(
      state,
      { connectionState: { cursor: event.id, status: "connected" } },
      effects
    );
  }
...
const selectScope = (
  ...
  return transition(
    ...
    [{ key: event.key, requestId: id, type: "load-analysis" }]
  );
};
export const deriveWorkspaceState = (
  state: LiveWorkspaceState
): LiveWorkspaceState => project(state, {});
// SAFETY: this reducer follows the standard (state, event) signature used by React-style reducers.
// oxlint-disable-next-line eslint/default-param-last
export const reduceWorkspace = (
  // SAFETY: reducer callers require state before event; the default is intentional.
  // oxlint-disable-next-line eslint/default-param-last
