...
export const ProcedureWorkspace = ({
  ...
  const handleFocus = (target: Parameters<WorkspaceController["focus"]>[0]) =>
    controller.focus(target);
  const handleImportsVisibleChange = (visible: boolean) =>
    controller.setImportsVisible(visible);
  return (
    <section className="flex min-h-0 flex-1 flex-col p-4 sm:p-6">
      ...
      <div className="relative min-h-0 flex-1">
          ...
          <div className="grid min-h-0 gap-3 xl:grid-cols-[1.6fr_1fr]">
            {analysis.cfg ? (
              <GraphPane
                ...
                onFocus={handleFocus}
                onImportsVisibleChange={handleImportsVisibleChange}
              />
            ) : (
              <section
                role="alert"
        ...
        {loading ? (
          <output className="absolute inset-0 z-10 flex items-start justify-center rounded-xl bg-[#07110E]/55 pt-4 backdrop-blur-[1px]">
