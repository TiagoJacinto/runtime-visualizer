...
const SelectField = ({
  ...
  <label className="block text-[9px] font-medium tracking-[0.16em] text-slate-500 uppercase">
    {label}
    <div className="relative mt-1.5">
      <select
        ...
        onChange={onChange}
        disabled={disabled}
        className="w-full appearance-none rounded-lg border border-white/10 bg-[#07110E] px-3 py-2.5 pr-8 text-xs text-slate-200 transition outline-none focus:border-emerald-300/50 focus:ring-2 focus:ring-emerald-300/10 disabled:cursor-not-allowed disabled:opacity-50"
      >
        {children}
      </select>
      <ChevronDown
        aria-hidden="true"
        className="pointer-events-none absolute top-3 right-2.5 h-3.5 w-3.5 text-slate-600"
      />
    </div>
  </label>
);
