...
const StatusIcon = ({ status }: { status: ExecutionRecord["status"] }) => {
  if (status === "running") {
...
const statusLabel = (status: ExecutionRecord["status"]): string =>
  status.charAt(0).toUpperCase() + status.slice(1);
const statusColor = (status: ExecutionRecord["status"]): string => {
  if (status === "running") {
    return "text-sky-300";
  }
  if (status === "failed") {
                ...
                <div className="mt-2 flex justify-end gap-1 pl-7">
                  ...
                  {execution.status === "running" ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded border border-amber-300/20 px-2 py-1 text-[9px] text-amber-200 hover:bg-amber-300/10 focus-visible:outline-2 focus-visible:outline-amber-200 disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      <X className="h-3 w-3" />
                      {cancelLabel}
                    </button>
