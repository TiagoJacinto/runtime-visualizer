...
export function FilePage() {
  ...
  const observedRun = runs[runs.length - 1]
  ...
  return (
    <div className="file-page">
      ...
      <section className="trace-section" aria-label="Trace">
        <h2>Trace</h2>
        {observedRun === undefined || observedRun.trace.length === 0 ? (
          <p className="muted">Run the procedure to observe its execution events.</p>
        ) : (
          <ol className="trace-list">
            {observedRun.trace.map((event, index) => (
              <li key={`${event.id ?? event.kind}-${index}`} className="trace-event">
                <span className="trace-kind">{event.kind}</span>
                <span className="trace-detail">
                  {event.kind === 'Branch'
                    ? `${event.condition} → ${event.outcome}`
                    : event.label}
                </span>
                <span className="trace-state">{event.state}</span>
              </li>
            ))}
          </ol>
        )}
        {observedRun?.resultStatus !== null && observedRun?.resultStatus !== undefined && (
          <p className={`execution-result ${observedRun.resultStatus === 'Succeeded' ? 'success' : 'failure'}`}>
            Result: {observedRun.resultStatus}
          </p>
        )}
      </section>
