...
**Exactly one question per message.** End every message with a single question - never a second question, a stacked follow-up, or an "and also...". Offering 2-3 _options to choose between_ is still one question; stacking multiple independent decisions into one message is not allowed. Walk down the decision tree one decision at a time: present the decision with its options, tradeoffs, and your recommendation, then stop and wait. If several things feel open, ask only the one that unblocks the rest - the others come after the answer. Never ask for a vague "any feedback?".
...
<instructions>
- Find the task directory: `ls -La .rpi/problems/TASKNAME` (use Bash `ls -La` - not Grep/Glob - the directory may be a symlink).
...
- Ground the design in the inputs you _do_ have, and cite them as you write: "Per the ticket...", "The research doc notes...", "As described in the PRD...".
...
<instructions>
Write the doc to `.rpi/problems/ENG-XXXX-description/NN-tdd-DESCRIPTION.md` (next zero-padded `NN-` index in the task dir; `DESCRIPTION` is a 2-4 word kebab-case slug).
...
## Required close-out views

End every Program Design with both views below.

**Test assertions** - state the important behavior each test proves and identify its test level or boundary. Specify assertions, not test implementation.

```text
integration: create resource
  boundary: API -> service -> store
  asserts: persists the resource and returns its generated ID
```

**Low-confidence decisions** - name the design choices the agent is least confident about and explain why they need human attention. Write `None` when there are no low-confidence decisions.

```text
- Queue retry policy - production traffic and failure-rate evidence are incomplete.
```

## HTML artifacts for complex concepts

The same HTML-artifact escape hatch from the System Design step applies here - use a focused inline HTML artifact when code shape, data flow, and annotations are clearer together than in plain text. Base any HTML artifact on `{SKILLBASE}/references/artifact_template.html` so it matches the RPI artifact viewer's visual language.
</guidance>
...
<instructions>
When both phases are approved, read the final answer template and follow it exactly (it points to the next step - the structure outline): `Read({SKILLBASE}/references/tdd_final_answer_resolved.md)`. Include RPI artifact permalinks if available.
</instructions>
