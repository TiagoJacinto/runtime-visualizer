...
1. **Read any directly mentioned files first:**
   ...
   - **DO NOT read problem files** - research must stay objective about the current codebase, not be influenced by what a ticket wants to build. The research questions already capture what needs to be investigated.
...
3. **Spawn parallel sub-agent tasks for comprehensive research:**
   ...
   **For web research and researching libraries & dependencies (if applicable):**
   ...
   - You _may_ see an RPI library-research tool. If you do, use it to answer in-scope questions about dependencies and libraries. If the tool is not present, do not mention it.
...
4. **Wait for all sub-agents to complete and synthesize findings:**
   ...
   - Verify all `.rpi/` paths are correct (task-specific files go in `.rpi/problems/`).
   - Highlight patterns, connections, and architectural decisions
   - Answer the user's specific questions with concrete evidence

5. **Gather metadata for the research document:**
   - Filename: `.rpi/problems/TASKNAME/NN-research-DESCRIPTION.md`
     - First, check the task directory in your system prompt. If you do not see it, find the task directory: `ls -La .rpi/problems | grep -i "eng-XXXX"`
     - If the directory doesn't exist, create: `.rpi/problems/ENG-XXXX-description/`
     ...
     - Directory naming:
       - With ticket: `.rpi/problems/ENG-1478-parent-child-tracking/02-research-parent-child-tracking.md`
       - Without ticket: `.rpi/problems/authentication-flow/02-research-auth-flow.md`
...
6. **Generate research document:**
   ...
   - Write the document to `.rpi/problems/TASKNAME/NN-research-DESCRIPTION.md`
...
8. **Note RPI artifact permalinks:**
   RPI artifact permalinks can be provided when you write artifacts. Include available permalinks in your final output.

   For code references in the current repository (if on the default branch or pushed):
...
9. **Respond to the user according to the template**
   ...
   - Respond following the template exactly, including RPI artifact permalinks if available.
...
header that asserts its _takeaway_ — the way a good slide title states its message ("Sessions
...
- **Path handling**: Task-specific research goes in `.rpi/problems/`.
  - Use `.rpi/problems/ENG-XXXX-description/NN-research-DESCRIPTION.md` for task research
