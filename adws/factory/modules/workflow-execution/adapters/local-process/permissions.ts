...
export function snapshot(run: any) {
  ...
  for (const line of snapshotGit(
    ["diff", "HEAD", "--numstat"],
    run.repoRoot,
  ).split("\n")) {
    const fields = line.split("\t");
    if (fields.length >= 3)
      out[fields.at(-1)!.trim()] = `${fields[0]},${fields[1]}`;
  }
  for (const path of snapshotGit(
...
function rollbackPath(run: any, path: string) {
  ...
  if (tracked) {
    ...
    const stillTracked =
      git(["ls-files", "--error-unmatch", "--", path], run.repoRoot).status ===
      0;
    if (stillTracked) {
...
export function enforce(
  ...
  const protectedFiles =
    agent.protectedFiles ?? run.cfg.defaults.protected_files ?? [];
  ...
  if (bad.length) {
    ...
    throw new PermissionBreach(
      `agent ${agent.name} changed unauthorized paths: ${bad.join(", ")}`,
    );
  }
}
