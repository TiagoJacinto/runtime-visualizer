...
import { AgentRuntime } from "../pi-agent/agent_runtime";
import { PiRequest, PiResult, usageZero } from "../pi-agent/runtime-types";
import { runProcess } from "../../../workflow-execution/process-runtime";
...
function isEnvelope(text: string) {
  ...
  return candidates.some((candidate) => {
    try {
      ...
      return Boolean(
        value && (value.status === "success" || value.status === "fail"),
      );
    } catch {
      return false;
    }
...
export async function run(
    ...
    if (event.type === "tool_use") {
      ...
      if (state.status === "completed" || state.status === "error") {
        ...
        const translated = {
          ...
          args,
          result: {
            content: [{ type: "text", text: String(state.output || "") }],
          },
          isError,
        };
        onEvent?.(translated);
  ...
  if (process.failure || process.exitCode !== 0)
    throw new Error(
      `opencode ${process.failure || `exit ${process.exitCode}`}`,
    );
